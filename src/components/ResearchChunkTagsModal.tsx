...
export default function ResearchChunkTagsModal({
    isOpen,
    onClose,
    notebookId,
    nexusId,
    ...
    const handleTagToggle = (tagId: string) => {
        const isCurrentlySelected = localSelectedTagIds.includes(tagId);
        const newSelectedIds = isCurrentlySelected
            ? localSelectedTagIds.filter(id => id !== tagId)
            : [...localSelectedTagIds, tagId];

        setLocalSelectedTagIds(newSelectedIds);
    };

    const handleSave = () => {
        onTagsChange(localSelectedTagIds);
        onClose();
    };
        ...
        return (
            <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {tags.map((tag) => {
                        const isSelected = localSelectedTagIds.includes(tag._id);
                        return (
                            <button
                                key={tag._id}
        ...
        <ModalBackground className="p-4">
            <div className="bg-gray-900 rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-700">
                    <div>
                        <h2 className="text-xl font-semibold text-white">Select Tags for Research Chunk</h2>
                        <p className="text-sm text-gray-400 mt-1">
                            {localSelectedTagIds.length} of {totalTags} tags selected
                        </p>
                        <p className="text-xs text-orange-400 mt-1">
                            Note: Tags will be applied when this chunk is saved to the notebook
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 border-t border-gray-700">
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={handleSave}
                            className="flex-1 px-4 py-2 bg-gradient-to-br from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 text-white rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                        >
                            Apply Tags
                        </button>
                    </div>
